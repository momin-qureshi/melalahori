...
router.get('/album/:locationId', (req, res, next)=>{
    Location.findOne({locationId: req.params.locationId}, function(err, location){
        res.json(location);
    });
});
...
router.get('/location/:type/:tag/:location', (req, res, next)=>{
    ...
    var location = (req.params.location=="any"?new RegExp('.*') : req.params.location);

    Location.find({type: type, tags: tag, location: location}, function(err, location){
        setTimeout((function() {res.json(location);}), 2000);
    });
});
...
router.post('/upload', (req, res)=>{
    upload(req, res, (err) => {
        if(err){
            ...
            Location.findOneAndUpdate({locationId: req.body.locationId}, { $push: { images: req.file.filename } }, (err, loc)=>{
                if(err){
                    console.log(err);
                }
            });
...
router.post('/addTag', (req, res)=>{
    Location.findOneAndUpdate({locationId: req.body.locationId}, { $push: { tags: req.body.tag } }, (err, loc)=>{
        if(err){
            console.log(err);
        }
        else{
